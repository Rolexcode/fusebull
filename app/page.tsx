...
const TG_URL = 'https://t.me/FuseBull';
const CONTRACT = '0x502C2c4B405f2e030C446c70A49db1Bd533B438B';
const steps = [
...
export default function Home(){
 ...
 return <main>
  ...
  <section className="hero" id="top"><div className="hero-grid"/><div className="hero-noise"/>
   ...
   <div className="hero-art"><div className="hero-halo"/><div className="hero-final-bull" role="img" aria-label="Fuse Bull standing after his charge, with cyan and violet visor and horns"/></div>
  </section>
  ...
  <section className="herd section-wrap" id="herd"><Label>004 / THE HERD</Label><h2>THE HERD<br/><span className="gradient-text">IS FORMING.</span></h2><p>Follow the Bull. Enter the Fuse.</p><div className="actions"><LinkButton href={X_URL}>FOLLOW $BULL ON X</LinkButton><LinkButton href={TG_URL} secondary>JOIN THE TELEGRAM</LinkButton><LinkButton href={FUSE_URL} secondary>VISIT FUSI.NG</LinkButton></div><div className="contract-line"><span>CONTRACT ADDRESS</span><code>{CONTRACT}</code><button type="button" onClick={async()=>{try{await navigator.clipboard.writeText(CONTRACT);setCopied(true);window.setTimeout(()=>setCopied(false),2200)}catch{setCopied(false)}}} aria-label="Copy contract address">{copied?'COPIED':'COPY'}</button></div><div className="herd-art"><BullImage/></div></section>
  <footer><div className="footer-main"><a href="#top">FUSE BULL<span>THE BULL OF FUSE.</span></a><div><a href={X_URL} target="_blank" rel="noopener noreferrer">X ↗</a><a href={TG_URL} target="_blank" rel="noopener noreferrer">TELEGRAM ↗</a><a href={FUSE_URL} target="_blank" rel="noopener noreferrer">FUSI.NG ↗</a></div></div><div className="footer-bottom"><span>CONCEPT INTERFACE / 2026</span><span>Tokens can be volatile. Nothing here is financial advice.</span></div></footer>
 </main>
}
